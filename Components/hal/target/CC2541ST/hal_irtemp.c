/**************************************************************************************************
  ...
  Revised:        $Date: 2012-09-25 06:25:32 -0700 (Tue, 25 Sep 2012) $
  Revision:       $Revision: 31616 $
...
#define TMP006_VAL_CONFIG_ON            0x7400  // Sensor on state
#define TMP006_VAL_CONFIG_OFF           0x0000  // Sensor off state
#define TMP006_VAL_MANF_ID              0x5449
#define TMP006_VAL_PROD_ID              0x0067
...
static uint8 configSensorOn[2] =  {0x74, 0x00};    // One coversion a second (4 average), continuous mode
...
bool HalIRTempRead(uint8 *pBuf)
{
  ...
  success = HalSensorReadReg(TMP006_REG_ADDR_VOLTAGE, (uint8 *)&voltage,IRTEMP_REG_LEN );
  if (success)
  {
    success = HalSensorReadReg(TMP006_REG_ADDR_TEMPERATURE, (uint8 *)&temperature,IRTEMP_REG_LEN );
  }

  if (success)
  {
    // Store values
    pBuf[0] = HI_UINT16( voltage );
    pBuf[1] = LO_UINT16( voltage );
    pBuf[2] = HI_UINT16( temperature );
    pBuf[3] = LO_UINT16( temperature );
  }
