...
bool HalHumiReadMeasurement(uint8 *pBuf)
{
  ...
  // Read temperature + CRC
  cmd = SHT21_CMD_TEMP_T_H;
  if (!HalHumiWriteCmd(cmd))
  {
    return FALSE;
  }

  success = HalHumiReadData(buf, sizeof(buf));
  if (success)
  {
    // Store temperature
    pBuf[0] = buf[1];
    ...
    // Read humidity + CRC
    cmd = SHT21_CMD_HUMI_T_H;
    success = HalHumiWriteCmd(cmd);
