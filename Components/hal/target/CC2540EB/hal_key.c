...
#define HAL_KEY_SW_3_ICTLBIT            BV(1) /* P0IEN - P0.1 enable/disable bit */
...
#define HAL_KEY_SW_4_ICTLBIT            BV(4) /* P0IEN - P0.1 enable/disable bit */
#define HAL_KEY_SW_4_IENBIT             BV(5) /* Mask bit for all of Port_0 */
#define HAL_KEY_SW_4_PXIFG              P0IFG /* Interrupt flag at source */

#define HAL_P0_EDGEBIT  BV(0)             /* PORT0中断触发边缘控制位  PICTL.P0ICON 1:下降沿 0:上升沿 */
#define HAL_P1L_EDGEBIT  BV(1)                           /* PORT1低四位中断触发边缘控制位 PICTL.P1ICONL */
#define HAL_P1H_EDGEBIT  BV(2)                          /* PORT1高四位中断触发边缘控制位 PICTL.P1ICONH */
#define HAL_P2_EDGEBIT  BV(3)             /* PORT2中断触发边缘控制位 PICTL.P2ICON */

#define  HAL_KEY_SW_1_EDGEBIT     HAL_P1L_EDGEBIT
#define  HAL_KEY_SW_234_EDGEBIT   HAL_P0_EDGEBIT
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
    ...
    PICTL |= (HAL_KEY_SW_1_EDGEBIT | HAL_KEY_SW_234_EDGEBIT);   /* Set the edge bit to set falling edge to give int */
//    PICTL &= ~(HAL_KEY_SW_1_EDGEBIT | HAL_KEY_SW_234_EDGEBIT);   /* Set the edge bit to set rasing edge to give int */
...
void halProcessKeyInterrupt (void)
{
  ...
  if( HAL_KEY_SW_3_PXIFG & HAL_KEY_SW_3_BIT) /* Interrupt Flag has been set by SW1 */
  {
...
HAL_ISR_FUNCTION( halKeyPort0Isr, P0INT_VECTOR )
{
  ...
  //          HalLedSet( HAL_LED_2 | HAL_LED_4, HAL_LED_MODE_ON );

#if defined ( CC2540_MINIDK )
  if ((HAL_KEY_SW_2_PXIFG & HAL_KEY_SW_2_BIT)        \
        || (HAL_KEY_SW_3_PXIFG & HAL_KEY_SW_3_BIT) || (HAL_KEY_SW_4_PXIFG & HAL_KEY_SW_4_BIT))
#else
