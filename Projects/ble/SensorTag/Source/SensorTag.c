/**************************************************************************************************
  ...
  Revised:        $Date: 2012-12-11 11:25:12 -0800 (Tue, 11 Dec 2012) $
  Revision:       $Revision: 32554 $
...
// How often to perform periodic events (milliseconds)
#define ST_IRTEMPERATURE_SENSOR_EVT_PERIOD               1000
#define ST_HUMIDITY_SENSOR_EVT_PERIOD                    1000
#define ST_BAROMETER_SENSOR_EVT_PERIOD                   1000
#define ST_BAROMETER_SENSOR_FSM_PERIOD                     80  // Delay after start of conversion
#define ST_MAGNETOMETER_SENSOR_EVT_DEFAULT_PERIOD        2000
#define ST_ACCELEROMETER_SENSOR_EVT_DEFAULT_PERIOD       1000
#define ST_GYRO_SENSOR_EVT_DEFAULT_PERIOD                1000

// Lower limits
#define ST_ACC_PERIOD_MIN                                 100
#define ST_ACC_TIME_UNIT                                   10 // resolution 10 ms
#define ST_MAG_PERIOD_MIN                                 100
#define ST_MAG_TIME_UNIT                                   10 // resolution 10 ms
...
static uint16 sensorMagPeriod = ST_MAGNETOMETER_SENSOR_EVT_DEFAULT_PERIOD;
static uint16 sensorAccPeriod = ST_ACCELEROMETER_SENSOR_EVT_DEFAULT_PERIOD;
...
uint16 SensorTag_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & ST_IRTEMPERATURE_READ_EVT )
  {
    if ( irTempEnabled )
    {
      ...
      // Restart timer
      osal_start_timerEx( sensorTag_TaskID, ST_IRTEMPERATURE_READ_EVT, ST_IRTEMPERATURE_SENSOR_EVT_PERIOD );
    }
    else
    {
  ...
  if ( events & ST_ACCELEROMETER_SENSOR_EVT )
  {
    if(accEnabled)
    {
      if (HalAccStatus() == KXTI9_ON)
      {
        readAccData();
      }
      else if (HalAccStatus() == KXTI9_OFF)
      {
        //Initiate Accelerometer sensor
        VOID HalAccTurnOn();
      }

      Accel_GetParameter( ACCELEROMETER_PERI, &sensorAccPeriod );
    ...
    else
    {
      ...
      VOID resetCharacteristicValue( ACCELEROMETER_SERV_UUID, ACCELEROMETER_PERI, ST_ACCELEROMETER_SENSOR_EVT_DEFAULT_PERIOD/ST_ACC_TIME_UNIT,
                             sizeof ( uint8 ));
    }
  ...
  if ( events & ST_HUMIDITY_SENSOR_EVT )
  {
    ...
    {
      readHumData();
      osal_start_timerEx( sensorTag_TaskID, ST_HUMIDITY_SENSOR_EVT, ST_HUMIDITY_SENSOR_EVT_PERIOD );
    }
    else
    {
  ...
  if ( events & ST_MAGNETOMETER_SENSOR_EVT )
  {
    if(magEnabled)
    {
      ...
      Magnetometer_GetParameter( MAGNETOMETER_PERI, &sensorMagPeriod );
    ...
    else
    {
      ...
      resetCharacteristicValue( MAGNETOMETER_SERV_UUID, MAGNETOMETER_PERI, ST_MAGNETOMETER_SENSOR_EVT_DEFAULT_PERIOD/ST_MAG_TIME_UNIT,
                               sizeof ( uint8 ));
    }
  ...
  if ( events & ST_BAROMETER_SENSOR_EVT )
  {
    if (barEnabled)
    {
      if (!barBusy)
      {
        ...
        osal_start_timerEx( sensorTag_TaskID, ST_BAROMETER_SENSOR_EVT, ST_BAROMETER_SENSOR_FSM_PERIOD );
      }
      else
      {
        barBusy = FALSE;
        readBarData();
        osal_start_timerEx( sensorTag_TaskID, ST_BAROMETER_SENSOR_EVT, ST_BAROMETER_SENSOR_EVT_PERIOD );
      }
    }
    else
    {
  ...
  if ( events & ST_GYROSCOPE_SENSOR_EVT )
  {
    if(gyroEnabled)
    {
      if (HalGyroStatus() == HAL_GYRO_STATE_RUNNING)
      {
      ...
      else if (HalGyroStatus() == HAL_GYRO_STATE_STOPPED)
      {
        HalGyroSelectAxes(sensorGyroAxes);
        HalGyroTurnOn();
      }
      osal_start_timerEx( sensorTag_TaskID, ST_GYROSCOPE_SENSOR_EVT, ST_GYRO_SENSOR_EVT_DEFAULT_PERIOD );
    }
    else
    {
      HalGyroTurnOff();
      if (HalGyroStatus()==HAL_GYRO_STATE_STOPPED)
      {
...
static void resetSensorSetup (void)
{
  ...
  if (HalAccStatus()!=KXTI9_OFF || accEnabled)
  {
    HalAccTurnOff();
    accEnabled = FALSE;
  }
  ...
  if (HalGyroStatus()!=HAL_GYRO_STATE_RUNNING || gyroEnabled)
  {
    HalGyroTurnOff();
    gyroEnabled = FALSE;
  }
...
static void peripheralStateNotificationCB( gaprole_States_t newState )
{
  switch ( newState )
  {
    ...
    case GAPROLE_WAITING:
      // Link terminated intentionally: turn off all sensors
      resetSensorSetup();
      break;
...
static void accelChangeCB( uint8 paramID )
{
  ...
  switch (paramID)
  {
    ...
    case ACCELEROMETER_PERI:
      ...
      if ( newValue >= ST_ACC_PERIOD_MIN/ST_ACC_TIME_UNIT )
      {
        sensorAccPeriod = (uint8)newValue*ST_ACC_TIME_UNIT;
      } // else: value must be above 100 ms
      break;
...
static void magnetometerChangeCB( uint8 paramID )
{
  ...
  switch (paramID)
  {
    ...
    case MAGNETOMETER_PERI:
      ...
      if ( newValue >= ST_MAG_PERIOD_MIN/ST_MAG_TIME_UNIT )
      {
        sensorMagPeriod = (uint8)newValue*ST_MAG_TIME_UNIT;
      } // else: value must be above 100 ms
      break;
...
static void resetCharacteristicValues( void )
{
  ...
  resetCharacteristicValue( ACCELEROMETER_SERV_UUID, ACCELEROMETER_PERI, (ST_ACCELEROMETER_SENSOR_EVT_DEFAULT_PERIOD)/10, sizeof ( uint8 ));
  ...
  resetCharacteristicValue( MAGNETOMETER_SERV_UUID, MAGNETOMETER_DATA, 0, MAGNETOMETER_DATA_LEN);
  resetCharacteristicValue( MAGNETOMETER_SERV_UUID, MAGNETOMETER_CONF, ST_CFG_SENSOR_DISABLE, sizeof ( uint8 ));
  resetCharacteristicValue( MAGNETOMETER_SERV_UUID, MAGNETOMETER_PERI,  (ST_MAGNETOMETER_SENSOR_EVT_DEFAULT_PERIOD)/10, sizeof ( uint8 ));
