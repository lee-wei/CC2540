...
void SimpleBLECentral_Init( uint8 task_id )
{
  ...
  test_InitService();           /* add test profile */
...
static void simpleBLECentral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  #else
  if ( keys & HAL_KEY_SW_1 )                    /* connect */
        {
            uint8 addrType = 0;
        unsigned char client_addr[B_ADDR_LEN] = {0xc2,0x88,0x8f,0xea,0x17,0x00}; 

            GAPCentralRole_EstablishLink( DEFAULT_LINK_HIGH_DUTY_CYCLE,
                                      DEFAULT_LINK_WHITE_LIST,
                                      addrType, client_addr );
  }

  if ( keys & HAL_KEY_SW_2 )                    /* turn on led */
  {
        attWriteReq_t req;
  ...
  if ( keys & HAL_KEY_SW_3 )                    /* turn off led */
  {
        attWriteReq_t req;
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        ...
        {
  //        if ( simpleBLEFindSvcUuid( SIMPLEPROFILE_SERV_UUID,                                 /* modify by liwei */
        if ( simpleBLEFindSvcUuid( TEST_SERVICE_UUID,  
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        {       
        HalLedSet( (HAL_LED_1 | HAL_LED_2 | HAL_LED_3 | HAL_LED_4), HAL_LED_MODE_BLINK);
...
static void simpleBLECentralStartDiscovery( void )
{
//  uint8 uuid[ATT_BT_UUID_SIZE] = { LO_UINT16(SIMPLEPROFILE_SERV_UUID),                /* modify by liwei */
 //                                  HI_UINT16(SIMPLEPROFILE_SERV_UUID) };
   uint8 uuid[ATT_BT_UUID_SIZE] = { LO_UINT16(TEST_SERVICE_UUID),                /* modify by liwei */
...
//        req.type.uuid[0] = LO_UINT16(SIMPLEPROFILE_CHAR1_UUID);               /* modify by liwei */
//        req.type.uuid[1] = HI_UINT16(SIMPLEPROFILE_CHAR1_UUID);

        req.type.uuid[0] = LO_UINT16(ENABLE_SERVICE_UUID);               /* modify by liwei */
        req.type.uuid[1] = HI_UINT16(ENABLE_SERVICE_UUID);
 ...
 * @return  none
 */
static uint8 ClientAddr[B_ADDR_LEN] = {0};
...
static void simpleBLEAddDeviceInfo( uint8 *pAddr, uint8 addrType )
{
  ...
  if ( simpleBLEScanRes < DEFAULT_MAX_SCAN_RES )
  {
    ...
    for ( i = 0; i < simpleBLEScanRes; i++ )
    {
      ...
      {
       //osal_memcpy( ClientAddr, pAddr, B_ADDR_LEN );
        return;
      }
    }
