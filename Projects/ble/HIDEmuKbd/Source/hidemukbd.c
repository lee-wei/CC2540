...
#define DEFAULT_HID_IDLE_TIMEOUT              20000
...
// Test values for feature report
static uint8 hidFeatureVal[] = "feature test";
static uint8 hidFeatureLen = sizeof(hidFeatureVal);
static uint8 hidFeatureMaxLen = sizeof(hidFeatureVal);
...
static uint8 hidEmuKbdRptCB( uint8 id, uint8 type, uint8 oper, uint8 *pLen, uint8 *pData );
...
static uint8 hidEmuKbdRptCB( uint8 id, uint8 type, uint8 oper, uint8 *pLen, uint8 *pData )
{
  ...
  if ( oper == HID_DEV_OPER_WRITE )
  {
    ...
    {
      if ( *pLen <= hidFeatureMaxLen )
      {
        osal_memcpy( hidFeatureVal, pData, *pLen );
        hidFeatureLen = *pLen;
      }
      else
      {
  ...
  else if ( oper == HID_DEV_OPER_READ )
  {
    ...
    {
      *pLen = hidFeatureLen;
      osal_memcpy(pData, hidFeatureVal, hidFeatureLen);
    }
    else
    {
      // read of input and output reports are possible but this implementation
      // does not use it; return zero-length data
      *pLen = 0;
    }
  }
