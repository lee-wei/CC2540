                         ...
                         uint8 *pValue, uint8 *pLen, uint16 offset, uint8 maxLen )
{
       ...
       uuid == HID_BOOT_MOUSE_INPUT_UUID )
  {
    ...
    if ( (pRpt = hidDevRptByHandle(pAttr->handle)) != NULL )
    {
      ...
      status  = (*pHidDevCB->reportCB)( pRpt->id, pRpt->type, HID_DEV_OPER_READ,
                                        pLen, pValue );
    }
    else
    {
                              ...
                              uint8 *pValue, uint8 len, uint16 offset )
{
       ...
       uuid == HID_BOOT_KEY_OUTPUT_UUID )
  {
    ...
    {
      // execute report callback
      status  = (*pHidDevCB->reportCB)( pRpt->id, pRpt->type, HID_DEV_OPER_WRITE,
                                        &len, pValue );
    }
  }
  ...
  else if ( uuid == GATT_CLIENT_CHAR_CFG_UUID )
  {
    ...
    if ( status == SUCCESS )
    {
      ...
      {
        // execute report callback
        (*pHidDevCB->reportCB)( pRpt->id, pRpt->type,
...
static void hidDevSendReport( uint8 id, uint8 type, uint8 len, uint8 *pData )
{
  ...
  if ( (pRpt = hidDevRptById(id, type)) != NULL )
  {
    ...
    if ( (pAttr = GATT_FindHandle(pRpt->cccdHandle, &retHandle)) != NULL )
    {
      ...
      {
        // send notification
        noti.handle = pRpt->handle;
        noti.len = len;
        osal_memcpy(noti.value, pData, len);
